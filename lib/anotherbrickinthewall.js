...
module.exports.createPost=function createPost (username, text, headline, newurl,cb) {
  pg.connect(DATABASE, function (error, client, done) {
    if (error) {
      return cb(error);
    }

    var values = [username, text, headline, newurl,new Date()];
    var query = 'INSERT into posts (username, text, headline, url,date) VALUES($1, $2, $3, $4, $5)';
...
module.exports.postC = function postC (id, cb) {
  pg.connect(DATABASE, function (error, client, done) {
    ...
    var values = [id];
    var query = 'SELECT headline, text, username, date, url FROM posts WHERE post_id=$1';
    client.query(query, values,function (err, result) {
      done();
