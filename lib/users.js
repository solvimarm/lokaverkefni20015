...
module.exports.updateprofile =function updateprofile (username, about, image, phone, email, rank,cb) {
  pg.connect(DATABASE, function (error, client, done) {
    if (error) {
      return cb(error);
    }

    var values = [username, about, image, phone, email, rank];
    var query = 'UPDATE profile SET email=$5, about=$2, image=$3, phonenumber=$4,rank=$6 WHERE username=$1';
    client.query(query, values, function (err, result) {
      done();
