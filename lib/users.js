...
module.exports.updateprofile =function updateprofile (username, about, image, phone, email, cb) {
  pg.connect(DATABASE, function (error, client, done) {
    if (error) {
      return cb(error);
    }

    var values = [username, about, image, phone, email];
    var query = 'UPDATE profile SET email=$5, about=$2, image=$3, phonenumber=$4 WHERE username=$1';
    client.query(query, values, function (err, result) {
      done();
